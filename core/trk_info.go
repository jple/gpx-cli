...
// Section is to contain Trkpt between two Trkpt.Name
...
func (trk Trk) GetFlattenTrkpts() Section {
	trkpts := Section{}
...
func (trk Trk) GetSections() Sections {
	return trk.GetFlattenTrkptsSplitName()
}

// GetFlattenTrkptsSplitName creates Sections ([][]Trkpt) containing Section ([]Trkpt until name (excluded))
func (trk Trk) GetFlattenTrkptsSplitName() Sections {
	sections := Sections{}
	s := Section{}
	...
	for i, trkpt := range trkpts {
		if i == 0 || trkpt.Name == nil {
			...
			// Update list of sections, starting new section
			sections = append(sections, s)
			s = Section{trkpt}
		}
	}
	sections = append(sections, s) // add last section
	return sections
}

// WIP: refacto GetInfo
func (trk Trk) GetInfoFromSections(trkid int, vitessePlat float64, detail bool) TrkSummary {
	sections := trk.GetSections()
	...
	for _, s := range sections {
		sectionSummary.NPoints = len(s)
		sectionSummary.Distance = GetCumDistance(s)
