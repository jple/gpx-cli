...
func TestBuildTrends2(t *testing.T) {
	var tests = []struct {
		have, want Trends
	}{
		{
			BuildTrends([]float64{100, 200, 300, 200, 100}, 30),
			NewTrends([]int{0, 2, 4},
				[]float64{100, 300, 100}),
		}, {
			BuildTrends([]float64{100, 200, 100, 200, 100}, 30),
			NewTrends([]int{0, 1, 2, 3, 4},
				[]float64{100, 200, 100, 200, 100}),
		}, {
			// must ignore change
			BuildTrends([]float64{100, 200, 180, 200, 100}, 30),
			NewTrends([]int{0, 1, 4},
				[]float64{100, 200, 100}),
		}, {
			// must update value
			BuildTrends([]float64{100, 200, 180, 220, 100}, 30),
			NewTrends([]int{0, 3, 4},
				[]float64{100, 220, 100}),
		},
	}
